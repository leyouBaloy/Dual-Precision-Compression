...
n_component = 2
n_prob_sample = 5
filename = f"HYCOM_WTS_hp"


# 设置日志记录器，在文件名中加入时间戳
logging.basicConfig(filename=f'saves/comparison/{filename}.log', level=logging.INFO, format='%(message)s')


if __name__ == "__main__":
    logging.info("loading data")
    dataset = HYCOM("/home/bailey/dataset/HYCOM/WTS")  # 改
    train_dataset = HYCOM_Train("/home/bailey/dataset/HYCOM/WTS", time=16)  # 改
    ...
    # 加载lpmodel的权重
    lp_save_dict = torch.load("saves/comparison/HYCOM_WTS_lp_ep100.pth")
    lpmodel.load_state_dict(lp_save_dict['model'])

    # 加载hpmodel的权重
    hp_save_dict = torch.load("saves/comparison/HYCOM_WTS_hp_ep130.pth")
    ...
    for ep in range(epochs):
        ...
        if (ep + start_ep) % save_interval == 0:
            ...
            utils.save_model(ep=ep + start_ep, model=hpmodel, model_name=f'comparison/{filename}_ep{ep + start_ep}.pth')
