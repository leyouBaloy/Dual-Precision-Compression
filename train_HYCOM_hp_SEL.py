...
if __name__ == "__main__":
    ...
    dataset = HYCOM("/home/bailey/dataset/HYCOM/SEL")  # 改
    train_dataset = HYCOM_Train("/home/bailey/dataset/HYCOM/SEL", time=16)  # 改
    ...
    # 加载lpmodel的权重
    lp_save_dict = torch.load("saves/comparison/HYCOM_SEL_lp_ep300.pth")
    lpmodel.load_state_dict(lp_save_dict['model'])

    # 加载hpmodel的权重
    hp_save_dict = torch.load("saves/comparison/HYCOM_SEL_hp_ep260.pth")
    ...
    for ep in range(epochs):
        ...
        # 每隔10个 epoch 保存一次模型
        save_interval = 10
        if (ep + 261) % save_interval == 0:
            logging.info(f"Saving model at epoch {ep + 261}...")
            utils.save_model(ep=ep + 261, model=hpmodel, model_name=f'comparison/{filename}_ep{ep + 261}.pth')
