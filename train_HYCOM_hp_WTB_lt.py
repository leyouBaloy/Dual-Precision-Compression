...
n_component = 2
n_prob_sample = 5
filename = f"hp_tain16"
save_base_path = './saves/ablation/WTB_lt_train16'
...
if __name__ == "__main__":
    ...
    dataset = HYCOM("/home/bailey/dataset/HYCOM/WTB_lt")  # 改
    train_dataset = HYCOM_Train("/home/bailey/dataset/HYCOM/WTB_lt", start_time=8, time=122)  # 改
    ...
    hpmodel = HPModel(num_hiddens=64, num_residual_layers=5, num_residual_hiddens=32,
                      num_embeddings=num_embeddings, embedding_dim=embedding_dim,
                      commitment_cost=0.25, decay=0.99, n_component=n_component).to(device)
    # 加载lpmodel的权重
    lp_save_dict = torch.load("saves/ablation/WTB_lt_train16/lp_train16.pth")
    lpmodel.load_state_dict(lp_save_dict['model'])

    # 加载hpmodel的权重
    hp_save_dict = torch.load("saves/ablation/HYCOM_WTB_lt_partition/HYCOM_WTB_lt_hp_ga_part2_ep460.pth")
    ...
    for ep in range(epochs):
        ...
        for idx, (data, test, mask) in enumerate(dataloader):
            try:
                ...
                # 计算概率损失
                probloss = utils.estimate_prob_and_loss(means, stds, weights, res_quantized, quan_num=1000, skip=n_prob_sample, HW_size=256)
        ...
        start_ep = 1
        save_interval = 20
        if (ep + start_ep) % save_interval == 0:
            logging.info(f"Saving model at epoch {ep + start_ep}...")
            utils.save_model(ep=ep + start_ep, model=hpmodel, save_dir=save_base_path, model_name=f'{filename}_ep{ep + start_ep}.pth')

        # if np.mean(all_losses) < min_loss:
        #     min_loss = np.mean(all_losses)
        #     logging.info("save model ...")
        #     utils.save_model(ep=ep + 0, model=hpmodel, model_name=f'comparison/{filename}.pth')  # 改2处